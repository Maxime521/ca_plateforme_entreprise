...
export default function CompanyDetailsPage() {
  ...
  const generateINSEEReport = async () => {
    try {
      const response = await fetch(`/api/documents/preview-insee?siren=${company.company.siren}`);
      
      if (response.ok) {
        // Open the HTML preview in a new tab
        const url = `/api/documents/preview-insee?siren=${company.company.siren}`;
    ...
    <Layout>
      <div className="py-8 px-4 sm:px-6 lg:px-8 min-h-screen">
        <div className="max-w-7xl mx-auto">
          ...
          {/* Documents Section - Moved to Top */}
          {/* <div className="mb-8">
          ...
          <div className="space-y-8">
            ...
            {activeTab === 'overview' && (
              <CompanyOverviewTab 
                company={{...companyData, siren: companyData.siren || siren}} 
                onViewDocuments={() => setActiveTab('documents')}
                onViewEstablishments={() => setActiveTab('establishments')}
                onViewBODACCReport={handleViewBODACCReport}
              />
            )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white dark:bg-dark-surface rounded-lg p-4 border border-primary-200 dark:border-primary-700">
                      ...
                      <button
                        onClick={async () => {
                          try {
                            const response = await fetch(`/api/documents/download/bodacc/${companyData.siren}?format=pdf`);
                            if (!response.ok) throw new Error('Erreur lors du téléchargement du PDF BODACC');
                            const blob = await response.blob();
                            const url = window.URL.createObjectURL(blob);
                            const a = document.createElement('a');
                            a.href = url;
                            a.download = `BODACC_${companyData.siren}.pdf`;
                        ...
                        className="w-full mt-2 px-3 py-2 bg-orange-600 hover:bg-orange-700 text-white text-sm font-medium rounded-md transition-colors"
                      >
                        Télécharger PDF
                      </button>
                    </div>
                    ...
                    <div className="bg-white dark:bg-dark-surface rounded-lg p-4 border border-primary-200 dark:border-primary-700">
                      ...
                      <button
                        onClick={async () => {
                          try {
                            const response = await fetch(`/api/documents/download/insee/${companyData.siren}?format=pdf`);
                            if (!response.ok) throw new Error('Erreur lors du téléchargement du PDF INSEE');
                            const blob = await response.blob();
                            const url = window.URL.createObjectURL(blob);
                            const a = document.createElement('a');
                            a.href = url;
                            a.download = `INSEE_${companyData.siren}.pdf`;
                            document.body.appendChild(a);
                            a.click();
                            a.remove();
                            window.URL.revokeObjectURL(url);
                        ...
                        className="w-full mt-2 px-3 py-2 bg-blue-800 hover:bg-blue-900 text-white text-sm font-medium rounded-md transition-colors"
                      >
                        Télécharger PDF
                      </button>
                    </div>
...
function CompanyOverviewTab({ company, onViewDocuments, onViewEstablishments, onViewBODACCReport }) {
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-dark-surface rounded-xl border border-gray-200 dark:border-dark-border p-6">
          ...
          <div className="space-y-3">
            ...
            <button 
              onClick={onViewEstablishments}
              className="w-full px-4 py-2 border border-gray-300 dark:border-dark-border text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-dark-card rounded-lg transition-colors text-sm font-medium"
            >
              🏪 Voir établissements
            </button>
            <button 
              onClick={onViewBODACCReport}
              className="w-full px-4 py-2 border border-green-300 dark:border-green-600 text-green-700 dark:text-green-300 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg transition-colors text-sm font-medium"
            >
              📰 Rapport BODACC
            </button>
          </div>
