...
export default function PDFDocumentCard({ document, showPreview = true }) {
  ...
  const { downloadDocument, getDownloadProgress } = useDownloadManager();
  ...
  const handlePreview = async () => {
    if (document.type === 'insee') {
      ...
      // For BODACC, use the HTML preview endpoint for formatted announcements
      const previewUrl = `/api/documents/preview/bodacc/${document.siren}`;
      window.open(previewUrl, '_blank');
  ...
  const getPreviewUrl = (document) => {
    switch (document.type) {
      ...
      case 'bodacc':
        // For BODACC HTML preview, use the API
        return `/api/documents/preview/bodacc/${document.siren}`;
      default:
      ...
      {showPreview && document.available !== false && (
        <div className="mb-4">
          <div className="w-full h-32 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600 flex items-center justify-center relative overflow-hidden">
            {getPreviewUrl(document) && !imageError ? (
              <iframe
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  {imageError ? 'Erreur de chargement' : 'Aperçu non disponible'}
                </p>
                {imageError && (
                  <button
      ...
      <div className="mt-4 space-y-2">
        ...
        {document.available && (
          <button
            ...
            disabled={downloadingDirect || isLoading}
            className="w-full px-4 py-2 bg-green-500 hover:bg-green-600 text-white font-medium text-sm rounded-lg transition-all duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {downloadingDirect ? (
