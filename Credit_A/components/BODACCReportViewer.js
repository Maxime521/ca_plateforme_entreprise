  ...
  const generateReport = useCallback(async (type = 'html') => {
    ...
    try {
      const endpoint = type === 'pdf' 
        ? `/api/documents/download/bodacc/${siren}?format=pdf&view=inline`
        : `/api/documents/preview-bodacc?siren=${siren}`;
        
      const response = await fetch(endpoint);
      
      if (type === 'html') {
        // For HTML preview, we expect HTML content, not JSON
        if (response.ok) {
          // Create a blob URL for the HTML content
          const htmlBlob = await response.blob();
          const htmlUrl = URL.createObjectURL(htmlBlob);
          
          setReportUrl(htmlUrl);
          setReportType(type);
          setError(null);
          setNoRecords(false);
          return { success: true, file: { path: htmlUrl } };
        } else {
          // Handle error responses
          const errorText = await response.text();
          const errorMessage = response.status === 404 
            ? 'Aucune annonce BODACC trouvée pour ce SIREN'
        ...
        // For PDF, use direct endpoint URL instead of blob
        if (response.ok) {
          // Use the direct endpoint URL for PDF viewing
          const pdfUrl = endpoint;
          
          setReportUrl(pdfUrl);
          setReportType(type);
          setError(null);
          setNoRecords(false);
          return { success: true, file: { path: pdfUrl } };
        } else {
          // Handle error responses
          let errorMessage;
          try {
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex rounded-lg bg-gray-100 dark:bg-dark-card p-1">
                ...
                <button
                  ...
                  title="Génère un fichier PDF réel compatible Adobe Reader"
                >
                  📋 PDF
                </button>
              </div>
              ...
              {reportUrl && (
                <a
                  href={reportUrl}
                  download={`BODACC_${siren}.${reportType === 'pdf' ? 'pdf' : 'html'}`}
                  target="_blank"
            ...
            {isGenerating ? (
              ...
              <div className="h-full relative">
                ...
                <iframe
                  ...
                  title={`Rapport BODACC ${reportType.toUpperCase()} ${siren}`}
                  allow="fullscreen"
                />
              </div>
            ) : (
...
export function useBODACCReports() {
  ...
  const generateAndView = async (siren, type = 'html') => {
    try {
      const endpoint = type === 'pdf' 
        ? `/api/documents/download/bodacc/${siren}?format=pdf`
        : `/api/documents/preview-bodacc?siren=${siren}`;
      ...
      if (type === 'html') {
        ...
        // For PDF, we expect a downloadable HTML response
        if (response.ok) {
