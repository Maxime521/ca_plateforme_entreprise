...
class INSEEPDFSimple {
  ...
  async downloadINSEEPDF(siren, siret = null) {
    try {
      ...
      // Clean SIRET by removing any spaces
      const cleanSIRET = targetSIRET.replace(/\s/g, '');
      
      // Format for display only
      const formattedSIRET = this.formatSIRETWithSpaces(cleanSIRET);
      const pdfUrl = `https://api-avis-situation-sirene.insee.fr/identification/pdf/${cleanSIRET}`;
      
      console.log(`📄 Downloading INSEE PDF for SIRET: ${formattedSIRET} (display)`);
      ...
      const fileName = `INSEE_Avis_Situation_${siren}_${cleanSIRET}.pdf`;
