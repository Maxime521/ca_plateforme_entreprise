...
export default function ScalablePDFViewer({ 
  ...
  const getIframeUrl = useCallback(() => {
    // Check if this is actually a PDF file by URL or content type
    // BODACC "PDF" downloads are actually HTML files, so don't add PDF parameters
    if (type === 'pdf' && url && (url.includes('.pdf') || url.includes('application/pdf'))) {
          ...
          >
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-1">
                ...
                <a
                  href={url}
                  download={siren ? `BODACC_${siren}.${type === 'pdf' ? 'pdf' : 'html'}` : undefined}
                  target="_blank"
